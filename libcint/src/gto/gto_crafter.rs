...
impl CInt {
    ...
    pub fn eval_gto_with_args_f(&self, args: GtoArgs<f64>) -> Result<CIntOutput<f64>, CIntError> {
        ...
        match blksize {
            ...
            _ => return Err(cint_error!(InvalidValue, "Unsupported blksize {blksize} for eval_gto_with_args_f; supported values are 32, 48, 54, 64, 72, 96, 104, 128, 144, 192, 256, 384, and 512")),
        }
    }
...
impl CInt {
    ...
    pub fn eval_gto_with_args_spinor_f(&self, args: GtoArgs<Complex<f64>>) -> Result<CIntOutput<Complex<f64>>, CIntError> {
        match args.blksize {
            ...
            blksize => return Err(cint_error!(InvalidValue, "Unsupported blksize {blksize} for eval_gto_with_args_spinor_f; supported values are 32, 48, 54, 64, 72, 96, 104, 128, 144, 192, 256, 384, and 512")),
        }
    }
